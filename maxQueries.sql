...
-- SELECT r.user_id, m.runtime, m.movie_id 
-- FROM Rating r LEFT JOIN Movies m ON r.movie_id = m.id WHERE r.user_id = 'austenfan' GROUP BY r.user_id, m.runtime, m.movie_id HAVING max(m.runtime);